...
if False: # plot all utilities in each graph 
    ...
    for metric, world, world_size in iter_graphs(): 
        ...
        # plt.show()
        plt.savefig('avg_plots/equitable_%s_%s.png' % (short_world, legend_label_map[metric]))
        plt.close()
...
if True: # plot PoA for each graph 
    ...
    for metric, world, world_size in iter_graphs(): 
        ...
        plt.suptitle('%s, %s' % (legend_label_map[metric], short_world))
        # plt.show()
        plt.savefig('poa_plots/equitable_%s_%s.png' % (short_world, legend_label_map[metric]))
        plt.close()
